...
Convergence toward $\theta^{\star}\in \Omega^{\star}= \text{argmin}_{\theta} L(\theta)$
...
- We want theory of implicit regularization for SGD
...
- But perhaps $\pi(\theta,x)$  is scheming (misaligned) because favoured by training
- If no strong regularization for honest solutions then schemer more likely
- Hope 1: Understanding implicit bias help us detect scheming
...
Train on data generated by $y=Mx$ with quadratic loss, input covariance matrix $\Sigma_X = I$: 
$$\begin{equation*}
...
---
## Global minima
- If $d_l \geq d_L$ unique in function space for $W^{\star} = \Sigma_{YX}=M$ (OLS) with $\Sigma_{YX}$ input-output covariance
- Whole space of parameters mapping to solution (degenerate)
- Key geometric invariants under symmetries: rank patterns $r_{ij}=\text{rank}(W_i...W_j)$
- Can stratify degenerate parameters mapping to global minima into $O_{r_{ij}}$ 
- Give combinatorial description of global minima  
- Can be used to compute dimension of global minima (link with RLCT)
- See more at this [paper from Simon Pepin Lehalleur and Richárd Rimányi](https://arxiv.org/abs/2411.19920)
- Informally: the rank patterns quantify simplicity between degenerate solutions
...
Work in progress to get combinatorics of saddles with quiver representation theory
...
## First order critical points
 ...
 From [Achour et. al](https://arxiv.org/abs/2107.13289), there exists $S$ a subset indexing the left singular vectors of: 
...
- Discover geometric invariant by studying their symmetries
...
Stability threshold ensuring monotonous decrease of loss in convex optimization
...
Conserved quantities through the gradient flow: 
...
- For more see [The geometry of the deep linear network, Govid Menon](https://arxiv.org/abs/2411.09004)
---
# Initialization
---
...
- SGD: Sample batch from a dataset and compute batch-gradient
- Continuous model with $B_t$ brownian
$$
d\theta_t=-\nabla L(\theta_t)\,dt+\sqrt{\eta\,\Sigma(\theta_t)}\,dB_t,
$$
- Gradient noise covariance $\Sigma(\theta_t)$ depends on geometry: anisotropic and state dependent
...
- In general noise is anisotropic: introduces different implicit bias
